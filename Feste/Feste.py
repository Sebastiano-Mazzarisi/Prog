...
# Nome: Feste.py
# Data ultima modifica: 16/03/2026 
...
REPO_NAME = "Sebastiano-Mazzarisi/Prog"

if not GITHUB_TOKEN:
    print("⚠️ ATTENZIONE: Token GitHub non trovato! Assicurati di aver creato il file .env o configurato i Secrets.")
# -----------------------------------------------------

ICON_URL = "https://sebastiano-mazzarisi.github.io/Prog/Feste/Feste.png?v=13"
TXT_URL = "https://sebastiano-mazzarisi.github.io/Prog/Feste/Feste.txt"


def aggiorna_github():
    """
    Funzione per caricare HTML, TXT, CSV, Script e PNG su GitHub.
    """
    if not GITHUB_TOKEN:
    ...
    try:
        ...
        # File di testo: (percorso locale, percorso nel repo)
        files_to_upload = [
            (OUTPUT_FILE,    f"Feste/{OUTPUT_FILE}"),
            (OUTPUT_TXT,     f"Feste/{OUTPUT_TXT}"),
            (INPUT_FILE,     f"Feste/{INPUT_FILE}"),
            ("Feste.py",     "Feste/Feste.py"),
        ]

        for filename, repo_path in files_to_upload:
            try:
                print(f"   ⬆️ Sto caricando {repo_path}...")
                ...
                try:
                    ...
                    print(f"   ✅ {repo_path} aggiornato con successo!")
                except:
                    repo.create_file(repo_path, f"Creazione iniziale {repo_path}", contenuto)
                    print(f"   ✅ {repo_path} creato con successo!")
        ...
        try:
            png_local = "Feste.png"
            png_repo  = "Feste/Feste.png"
            if os.path.exists(png_local):
                print(f"   ⬆️ Sto caricando {png_repo}...")
                with open(png_local, 'rb') as f:
                    png_bytes = f.read()
                try:
                    contents = repo.get_contents(png_repo)
                    repo.update_file(contents.path, f"Aggiornamento {datetime.now()}", png_bytes, contents.sha)
                    print(f"   ✅ {png_repo} aggiornato con successo!")
                except:
                    repo.create_file(png_repo, "Creazione Feste.png", png_bytes)
                    print(f"   ✅ {png_repo} creato con successo!")
            else:
                print(f"   ⚠️ Feste.png non trovato in locale, salto l'upload.")
        except Exception as e:
            print(f"   ❌ Errore upload Feste.png: {e}")
